...
namespace CarDealershipASPNETMVC.Data
{
    public class DataAccessOrder : IDataAccessOrder
    {
        ...
        public DataAccessOrder(IConfiguration config)
        {
            _config = config;
                    ...
                    using (SqlCommand command = new SqlCommand("dbo.spOrdersSearchDynamicSQL", connection))
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                order.SaleTime = reader.IsDBNull(21) ? null : reader.GetDateTime(21);
                                order.ShoppingCartOrderId = reader.IsDBNull(22) ? null : reader.GetInt32(22);
                                order.ShoppingCartStatusName = reader.IsDBNull(23) ? null : reader.GetString(23);
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionStringCarDealerShipDB))
                {
                    ...
                    using (SqlCommand command = new SqlCommand("dbo.spOrdersSearchDynamicSQL", connection))
                    {
                        ...
                        command.Parameters.AddWithValue("@ShoppingCartOrderId", orderSearch.ShoppingCartOrderId);
                        command.Parameters.AddWithValue("@ShoppingCartStatusName", orderSearch.ShoppingCartStatusName);
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionStringCarDealerShipDB))
                {
                    ...
                    using (SqlCommand command = new SqlCommand("[dbo].[spOrdersUpdateOrInsert]", connection))
                    {
                        ...
                        command.Parameters.AddWithValue("@ShoppingCartOrderId", insertedOrder.ShoppingCartOrderId);
                        command.Parameters.AddWithValue("@ShoppingCartStatusName", insertedOrder.ShoppingCartStatusName);
        ...
        public async Task ShoppingCartStatusSettings(int ShoppingCartOrderId, string ShoppingCartStatusName)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionStringCarDealerShipDB))
                {
                    ...
                    using (SqlCommand command = new SqlCommand("spShoppingCartStatusSettings", connection))
                    {
                        command.CommandType = System.Data.CommandType.StoredProcedure;

                        command.Parameters.AddWithValue("@ShoppingCartOrderId", ShoppingCartOrderId);
