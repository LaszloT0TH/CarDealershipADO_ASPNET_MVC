...
{
    public class DataAccessCarAccessories : IDataAccessCarAccessories
    {
        static String connectionStringCarDealerShipDB = String.Empty;
        private readonly IConfiguration _config;
        string errorMessage = String.Empty;

        public DataAccessCarAccessories()
        {

        }
        public DataAccessCarAccessories(IConfiguration config)
        {
            _config = config;
                    ...
                    using (SqlCommand command = new SqlCommand("dbo.spCarAccessoriesSearchDynamicSQL", connection))
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                carAccessories.PhotoPath = reader.IsDBNull(14) ? null : reader.GetString(14);
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionStringCarDealerShipDB))
                {
                    ...
                    using (SqlCommand command = new SqlCommand("[dbo].[spCarAccessoriesUpdateOrInsert]", connection))
                    {
                        ...
                        command.Parameters.AddWithValue("@PhotoPath", insertedCarAccessories.PhotoPath);
        ...
        /// <summary>
        /// CarAccessoriesProductGroup for a drop-down menu list
        /// </summary>
        /// <returns></returns>
        // GetAllData, Search
        ...
        /// <summary>
        /// CarAccessoriesUnit for a drop-down menu list
        /// </summary>
        /// <returns></returns>
        // GetAllData, Search
